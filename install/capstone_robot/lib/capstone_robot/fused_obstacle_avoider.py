...
class FusedObstacleAvoider(Node):
    ...
    def publish_cmd(self):
        # Safety override: a person closer than STOP_DISTANCE always blocks
        # forward motion (no avoidance steering attempted around a person),
        # but reverse stays available -- otherwise an Ackermann-steered car
        # can deadlock, since it can't turn in place to escape without
        # rolling forward, which would be the only other way out.
