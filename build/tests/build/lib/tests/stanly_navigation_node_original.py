...
class NavigationAndControlNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Float32, '/steering_correction', self.steering_factor_callback, 10)
        ...
        self.obstacle_steering_angle = 0.0  # radians
        self.last_steering_time = self.get_clock().now()

        self.create_timer(0.2, self.navigate)  # 5 Hz
    ...
    def steering_factor_callback(self, msg: Float32):
        self.obstacle_steering_angle = msg.data  # in radians
        self.last_steering_time = self.get_clock().now()
        # self.get_logger().info(f"Received obstacle steering correction: {math.degrees(self.obstacle_steering_angle):.2f}°")
    ...
    def navigate(self):
        ...
        self.control_robot(stanley_steering)

    def control_robot(self, steering_angle):
        max_steering_units = 0.8
        max_steering_angle_deg = 35.0

        # Blend in obstacle avoidance steering (if recent)
        time_since_steer = (self.get_clock().now() - self.last_steering_time).nanoseconds * 1e-9
        if time_since_steer <= 1.0:
            correction_deg = math.degrees(self.obstacle_steering_angle)
        else:
            correction_deg = 0.0

        combined_steering_deg = steering_angle - correction_deg
        ...
        # Timeout safety for speed factor
