...
class NavigationAndControlNode(Node):
    def __init__(self):
        ...
        self.speed_factor = 1.0  # Default to no reduction
        ...
        self.waypoints = self.load_waypoints('/home/user/abd_ws_2/src/tests/tests/perimeter/H/border_path_1.csv')
        ...
        # Control parameter
        self.k = -2  # negative for right-hand drive coordinate frame
    ...
    def speed_factor_callback(self, msg: Float32):
        self.speed_factor = max(0.0, min(1.0, msg.data))  # Clamp between 0 and 1
        self.last_factor_time = self.get_clock().now()
    ...
    def control_robot(self, steering_angle):
        ...
        if time_since_last > 1.0:
            self.speed_factor = 1.0  # fallback
            self.get_logger().warn("No speed factor received recently. Defaulting to 1.0")


        twist = Twist()
        twist.linear.x = self.v * self.speed_factor  # Constant speed in m/s
